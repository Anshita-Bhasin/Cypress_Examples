...
class GridWorker extends _events.EventEmitter {
  constructor(clientSocket, params) {
    ...
    clientSocket.on('error', error => this.closeWorker(WSErrors.CLIENT_SOCKET_ERROR)); // clientSocket.pause() would be preferrable but according to the docs " Some events can still be
...
class GridServer {
  ...
  _createAgent(os) {
    ...
    const initPromise = Promise.resolve().then(() => this._factory.launch({
      ...
      this._log('failed to launch agent ' + agent.agentId); // eslint-disable-next-line no-console
